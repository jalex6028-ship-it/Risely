...
async function storeGet(k){try{const v=localStorage.getItem(k);return v?JSON.parse(v):null;}catch{return null;}}
async function storeSet(k,v){try{localStorage.setItem(k,JSON.stringify(v));}catch{}}
  ...
  <Modal title="Season Rankings" onClose={()=>setModal(null)} wide>
    ...
    {ranksTab==="leaderboard"&&(<>
      ...
      {byMonth.length>=3&&(
        <div style={{display:"flex",alignItems:"flex-end",justifyContent:"center",gap:8,marginBottom:20}}>
          {[byMonth[1],byMonth[0],byMonth[2]].map((m,pi)=>{
            const pos=[2,1,3][pi],lv=getLv(m.allTimeXP||0),H=[90,120,72][pi],isMe=m.id===me?.id;
      ...
      {byMonth.slice(3).map((m,i)=>{
        const lv=getLv(m.allTimeXP||0),isMe=m.id===me?.id;
        return(
...
function InteractApp({user, userRole, isAdmin: isAdminProp, realtimeData, onLogout}){
  const KEY="ixp_";
  const fileRef=useRef();

  // Initialize from real-time data props, with fallbacks to defaults
  const [members,    setMembers]    = useState(realtimeData?.members || DEFAULT_MEMBERS);
  const [quests,     setQuests]     = useState(realtimeData?.quests || DEFAULT_QUESTS);
  const [feed,       setFeed]       = useState(DEFAULT_FEED);
  const [challenges, setChallenges] = useState(realtimeData?.challenges || DEFAULT_CHALLENGES);
  const [polls,      setPolls]      = useState(realtimeData?.polls || DEFAULT_POLLS);
  const [events,     setEvents]     = useState(realtimeData?.events || DEFAULT_EVENTS);
  const [rewards,    setRewards]    = useState(realtimeData?.rewards || DEFAULT_REWARDS);
  const [daily,      setDaily]      = useState(realtimeData?.dailyTasks || DEFAULT_DAILY);
  ...
  // form state
  // OLD LOGIN STATE REMOVED - AuthWrapper handles authentication now
  ...
  // Helper to persist member changes to Supabase (handles field mapping from camelCase to snake_case)
  const persistMemberToSupabase = async (memberId, updates) => {
    ...
    if (Object.keys(supabaseUpdates).length > 0) {
      const { error } = await supabase.from('members').update(supabaseUpdates).eq('id', currentMbr.id);
      if (error) console.error('[PERSIST] Supabase update failed:', error);
    }
  };

  // Sync real-time data from Supabase props
  useEffect(() => {
    if (realtimeData?.members) setMembers(fin(realtimeData.members.map(convertSupabaseToMemory)));
    if (realtimeData?.quests) setQuests(realtimeData.quests);
    if (realtimeData?.challenges) setChallenges(realtimeData.challenges);
    if (realtimeData?.events) setEvents(realtimeData.events);
    if (realtimeData?.rewards) setRewards(realtimeData.rewards);
    if (realtimeData?.dailyTasks) setDaily(realtimeData.dailyTasks);
    if (realtimeData?.polls) setPolls(realtimeData.polls);
  }, [realtimeData?.members, realtimeData?.quests, realtimeData?.challenges, realtimeData?.events, realtimeData?.rewards, realtimeData?.dailyTasks, realtimeData?.polls]);

  useEffect(()=>{
    let cancelled=false;
    (async()=>{
      try{
        // Load feed, seasons, notifications from localStorage (local-only data)
        const f=await storeGet(KEY+"feed");
        const se=await storeGet(KEY+"seasons");
        const no=await storeGet(KEY+"notifs");
        const ar=await storeGet(KEY+"autorules");
        const lsm=await storeGet(KEY+"lastMonth");

        if(cancelled)return;

        const cur=monthStr();
        // Use members from state which is now synced from Supabase
        let lM=members;
  ...
  // OLD handleLogin REMOVED - AuthWrapper handles authentication now
  ...
  const handleLoginRewardClaim=async (reward)=>{
    ...
    const today=todayStr();
    const yesterday=new Date(Date.now()-86400000).toISOString().slice(0,10);
    if(me.lastLoginDate===today)return; // already claimed
    const isStreakAlive=me.lastLoginDate===yesterday;
    const newStreak=(isStreakAlive?(me.loginStreak||0):0)+1;
    let u=awardXPWithEffects(members,me.id,reward.xp,`Day ${newStreak%7||7} login reward`,reward.icon);
    u=u.map(m=>m.id===me.id?{...m,loginStreak:newStreak,lastLoginDate:today,loginHistory:[...(m.loginHistory||[]),today].slice(-30)}:m);
    ...
    if(updatedMember) {
      ...
      if (error) console.error('[DAILY-LOGIN] Supabase update failed:', error);
    }
  ...
  const handleGemCrushReward=(xpAmount, gcScore, newHiScore)=>{
    if(!me||!userId)return;
    let u=awardXPWithEffects(members,me.id,xpAmount,"GemCrush","gem");
    u=u.map(m=>m.id===me.id?{...m,
  ...
  const doDaily=taskId=>{
    if(!me||todayDone.includes(taskId))return;
    const t=daily.find(x=>x.id===taskId);
    const prevBadges=new Set(members.find(m=>m.id===me.id)?.badges||[]);
    let u=awardXPWithEffects(members,me.id,t.xp,t.title,"check");
    const newDone=[...todayDone,taskId];
    u=u.map(m=>m.id!==me.id?m:{...m,dailyDone:{...m.dailyDone,[todayStr()]:newDone}});
    const finU=fin(u);
    ...
    const newBadges=new Set(finU.find(m=>m.id===me.id)?.badges||[]);
  ...
  const doShoutout=()=>{
    ...
    const u=awardXPWithEffects(members,me.id,10,"Posted a shoutout","megaphone");
    const withBadge=u.map(m=>m.id===me.id?{...m,badges:[...new Set([...m.badges,"social_bee"])]}:m);
  ...
  const doVote=(pid,oid)=>{
    ...
    if(!me.badges.includes("voter")){
      const u=awardXPWithEffects(members,me.id,5,"Voted in a poll","vote");
      saveM(u.map(m=>m.id===me.id?{...m,badges:[...m.badges,"voter"]}:m));
  ...
  const doRedeem=rw=>{
    ...
    const prevBadges=new Set(me.badges||[]);
    const withBadge=fin(members.map(m=>m.id===me?.id?{...m,spentXP:(m.spentXP||0)+rw.cost,badges:[...new Set([...m.badges,"shopper"])]}:m));
    saveM(withBadge);
  ...
  const handlePhotoUpload=e=>{
    ...
    reader.onload=ev=>{ saveM(fin(members.map(m=>m.id===me?.id?{...m,photo:ev.target.result}:m))); notify("Photo updated"); };
  ...
  const saveProfile=()=>{
    ...
    saveM(fin(members.map(m=>m.id!==me?.id?m:{...m,name:editName.trim()||m.name,password:editPwNew||m.password})));
  ...
  const adminQuestDone=()=>{
    ...
    const finU = fin(u);
    const updatedMember = finU.find(m=>m.id===qTarget);
    if(updatedMember) {
    ...
    saveM(finU); const name=members.find(m=>m.id===qTarget)?.name;
  ...
  const adminChallDone=(chId,mId)=>{
    ...
    const finU = fin(u);
    const updatedMember = finU.find(m=>m.id===mId);
    if(updatedMember) {
    ...
    saveM(finU); const name=members.find(m=>m.id===mId)?.name;
  ...
  const aheadOf = me ? [...byMonth].reverse().find(m=>m.id!==me.id&&m.monthlyXP>(me.monthlyXP||0)) : null;
  ...
  const doMysteryBox = () => {
    ...
    setTimeout(()=>{
      ...
      const u2=members.map(m=>m.id!==me?.id?m:{...m,spentXP:(m.spentXP||0)+100});
      saveM(fin(u2));
    },1600);
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,color:TEXT,fontFamily:"'Sora',-apple-system,'Segoe UI',Roboto,sans-serif",paddingBottom:"calc(80px + env(safe-area-inset-bottom))"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            {me ? (
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <button className="btn-ghost" onClick={onLogout} style={{padding:"7px 14px",fontSize:12}}>
                  Sign out
                </button>
              </div>
            ) : null}
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"14px 14px"}} className="tab-in">
        {tab==="home"&&!me&&user&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"60vh",padding:"0 8px"}}>
            <div style={{
              width:40,height:40,border:"2px solid rgba(59,158,255,.2)",
              borderTop:"2px solid #3b9eff",borderRadius:"50%",
              margin:"0 auto",animation:"spin 0.8s linear infinite",
            }}/>
            <div style={{fontSize:14,color:"rgba(255,255,255,.6)",marginTop:16}}>Loading your profile...</div>
          </div>
        )}
        {tab==="home"&&me&&(()=>{
              ...
              <div style={{background:"#0c1525",border:"1px solid #182840",borderRadius:20,padding:"14px 16px",marginBottom:12}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button className="btn-primary" style={{flex:1}} onClick={()=>{
                    ...
                    saveF([post,...feed]);
                    if(!isAdmin&&me){
                      let u=awardXPWithEffects(members,me.id,5,"Posted to feed","heart");
                      const finU = fin(u);
                      const updatedMember = finU.find(m=>m.id===me.id);
                      if(updatedMember) {
                        persistMemberToSupabase(me.id, {
                          xp: updatedMember.xp,
                          monthlyXP: updatedMember.monthlyXP,
                          allTimeXP: updatedMember.allTimeXP,
                          xpHistory: updatedMember.xpHistory
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center"}}>
                        {!isPast&&me&&(
                          <button onClick={()=>{
                            ...
                            if(!going&&me){
                              let u=awardXPWithEffects(members,me.id,10,"RSVP to event","calendar");
                              const finU = fin(u);
                              const updatedMember = finU.find(m=>m.id===me.id);
                              if(updatedMember) {
                                persistMemberToSupabase(me.id, {
        ...
        {tab==="shop"&&(()=>{
          ...
          const MysteryBoxCard = () => (
            <div style={{padding:"6px 16px 4px"}}>
              ...
              }} className="shop-card-hover" onClick={me?doMysteryBox:undefined}>
                <div style={{position:"absolute",inset:0,pointerEvents:"none",
      ...
      {/* LOGIN MODAL REMOVED - Authentication now handled by AuthWrapper */}

      {/* ADMIN ACCESS CHECK - Now role-based instead of PIN */}
      {modal==="admin"&&!isAdmin&&(
        <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(20px)",background:"rgba(2,6,18,.92)"}}>
          <div style={{maxWidth:380,textAlign:"center",padding:"32px",background:"linear-gradient(135deg,#0b1728,#060e1d)",border:"1px solid rgba(248,113,113,.2)",borderRadius:20}}>
            <svg width={48} height={48} viewBox="0 0 24 24" fill="none" stroke="#f87171" strokeWidth="1.6" style={{margin:"0 auto 16px",display:"block"}}>
              <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
            </svg>
            <div style={{fontSize:18,fontWeight:900,color:"#fff",marginBottom:8}}>Access Denied</div>
            <div style={{fontSize:13,color:"rgba(255,255,255,.5)",marginBottom:20}}>Only admin and moderator users can access the board. Contact an administrator to request access.</div>
            <button onClick={()=>setModal(null)} style={{width:"100%",padding:"12px",background:"rgba(59,158,255,.1)",border:"1px solid rgba(59,158,255,.2)",borderRadius:12,color:"#3b9eff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              Close
            </button>
          </div>
        </div>
          ...
          {(()=>{
            ...
            return(
              <div>
                ...
                {detailM.id===me?.id&&<button className="btn-secondary" style={{width:"100%",marginTop:14}} onClick={()=>{setHistoryM(detailM);setDetailM(null);}}>View Diamond History</button>}
              </div>
            );
          })()}
        </Modal>
...
// Export with AuthWrapper to enable authentication and real-time sync
export default function Page() {
  return <AuthWrapper children={InteractApp} />;
}
                                            